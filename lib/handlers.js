            ...
            if (!err && userData) {
              const userChecks =
                ...
                userData.checks instanceof Array
                  ? userData.checks
                  : [];
              // Verify they have less than max number of checks
              if (userChecks.length < config.maxChecks) {
                // Create a random id for the check
                const checkId = helpers.createRandomString(20);
                // Create the check object, and include the user's user
                const checkObject = {
                  id: checkId,
                  userPhone,
                  protocol,
                  ...
                  if (!err) {
                    ...
                    _data.update('users', userPhone, userData, err => {
                      if (!err) {
                        ...
                        callback(500, {
                          Error: 'Could not update user Check.',
                        });
                      }
                    });
                  } else {
      ...
      _data.read('checks', id, (err, checkData) => {
        if (!err && checkData) {
          ...
          handlers._tokens.verifyToken(
            ...
            tokenIsValid => {
              if (tokenIsValid) {
                // Verify data was sent to update
                if (
                  protocol ||
                  url ||
                  method ||
                  ...
                  // Update the checks object with the new data
            ...
            tokenIsValid => {
              if (tokenIsValid) {
                ...
                _data.delete('checks', id, err => {
                  if (!err) {
                    // Delete the check reference in the user's object
                    _data.read(
                      'users',
                      checkData.userPhone,
                      (err, userData) => {
                        if (!err && userData) {
                          ...
                          if (checkPosition > -1) {
                            ...
                            _data.update(
                              ...
                              err => {
                                if (!err) {
                                  ...
                                  callback(500, {
                                    Error:
                                      'Could not update user object checks reference.',
                                  });
                                }
                              }
                            );
                            ...
                            callback(500, {
                              Error:
                                "Could not find the check reference on the user's account.",
                            });
                          }
                        } else {
                          callback(500, {
                            Error:
                              'Could not delete the stored check reference. No user found.',
                          });
                        }
                      }
                    );
                    ...
                    callback(500, {
                      Error: 'Could not delete the stored check.',
                    });
                  }
                });
              } else {
...
handlers._tokens = {
  ...
  PUT: (data, callback) => {
    ...
    if (id && extend) {
      ...
      callback(400, {
        Error: 'Missing required field(s) or payload invalid.',
      });
    }
  },
                      ...
                      _data.delete('checks', checkId, err => {
                        ...
                        if (checksDeleted === checksToDelete) {
                          if (!deletionErrors) {
                            ...
                            callback(500, {
                              Error: 'Checks not deleted successfully.',
                            });
                          }
                        }
                      });
