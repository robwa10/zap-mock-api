...
const workers = {
  alertUserToStatusChange: data => {
    ...
    helpers.sendTwilioSms(data.userPhone, msg, err => {
      if (!err) {
        debug('Successful alert: ', msg);
      } else {
        debug('Error: Could not send sms alert to check id: ', data.id);
      }
    });
  },
  ...
  log: logData => {
    ...
    _logs.append(logFileName, logString, err => {
      if (!err) {
        ...
        debug('Error writing log file.', logData);
      }
    });
  },
  ...
  rotateLogs: () => {
    ...
    _logs.list(false, (err, logs) => {
      if (!err && logs && logs.length > 0) {
        ...
        debugs('Error: Could not find logs to rotate.');
      }
    });
  },

  init: () => {
    // Log that workers have started in yellow
